...
Inputs:
  UNOHICK_Occupancy_20260227_235408.xlsx  — 2024
  UNOHICK_Occupancy_20260227_235324.xlsx  — 2025

Output:
  clean_occupancy.csv  — 638 rows × 37 columns, zero missing values

═══════════════════════════════════════════════════════════════════
RAW FILE FORMAT  (forensically verified — do not guess at this)
═══════════════════════════════════════════════════════════════════

Each block covers 5 dates (sometimes fewer at page/year boundaries):

  Row A:  "Room Type" | date1(col 1) | date2(col 11) | date3(col 17) | date4(col 23) | date5(col 29)
  Row B:  (nan)       | Avl Ov Let _ Tot _ Occ% _ Slprs  ×5 date-groups
  Rows C–F: room type data rows — DB, DB_SB, EXEC, TB
  Row G:  "Totals"    | aggregated values for each date

Column offsets per date (two layouts, confirmed from label row):
  Date 1  (base=1)   wider layout with NaN spacers:
      Avl=+0  Ov=+1  Let=+2  [NaN=+3]  Tot=+4  [NaN=+5]  Occ%=+6  [NaN=+7]  Slprs=+8
  Dates 2–5 (base=11/17/23/29) compact layout:
      Avl=+0  Ov=+1  Let=+2  Tot=+3  Occ%=+4  Slprs=+5

Room types per block:
  DB     — Double Bedroom        (base capacity ~36)
  DB_SB  — Double/Single Bed     (base capacity ~5)
  EXEC   — Executive Rooms       (base capacity ~6)
  TB     — Twin Bedroom          (base capacity ~5)
  Total hotel capacity: 52 rooms

═══════════════════════════════════════════════════════════════════
KNOWN RAW FILE COMPLICATIONS  (all handled below)
═══════════════════════════════════════════════════════════════════

1. PAGE BREAKS every ~30 rows inject two noise rows mid-block:
     "Page N of 19 at 23:53 on the 27/Feb/2026 | KHALIDSIOUD"
     "The Hickstead | Occupancy Report..."
   → Strip before scanning

2. EMPTY ROOM TYPE HEADERS after page breaks:
     "Room Type" row with no dates (col 1 = nan)
   These are page-continuation markers; the block's header was on the previous page.
   → Skip when scanning forward inside a block

3. SPLIT ROWS — label and data on separate rows, three sub-patterns:
   A. Label then data   (most common): "DB" row (no data) → next nan row has numbers
   B. Data then label   (seen in Jul+): nan data row → "DB" label row (no data)
   C. Label alone with  post-label data: "Totals" (no data) → next nan row 1–3 rows later

   Fix: queue-based scanner that buffers pre-label nan data rows,
        and uses lookahead for post-label data rows.

4. ANNUAL TOTALS BLOCK at end of file:
     "Room Type | Totals"  (col 1 = 'Totals' string)
   Different column layout — not daily data.
   → Skip when col 1 = 'Totals'

5. GENUINE PMS GAPS — 4 dates in December 2025 where the PMS block exists
   but the Totals row has no numeric data.  These are filled by interpolation.
